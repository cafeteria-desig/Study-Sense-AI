...
export function PdfPage() {
  ...
  return (
    <div className="h-[100dvh] bg-[#08080a] text-[#F4F2EC] flex flex-col overflow-hidden font-sans relative selection:bg-white/20 selection:text-[#08080a]">
      ...
      <header className="px-3 sm:px-6 py-2.5 sm:py-3 border-b border-white/10 bg-[#08080a]/90 backdrop-blur-xl sticky top-0 flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 z-30 shrink-0 w-full max-w-full overflow-hidden">
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          <Link
            to="/dashboard"
            className="inline-flex items-center gap-1 text-xs font-mono text-[#A6A49C] hover:text-[#F4F2EC] transition-colors p-1 shrink-0"
          >
          ...
          <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1 rounded-full border border-white/15 bg-white/5 backdrop-blur-md text-[10px] sm:text-[11px] font-semibold tracking-wider text-[#F4F2EC] shrink-0">
            <span className="relative flex h-2 w-2 items-center justify-center shrink-0">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-75" />
              <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-white" />
            </span>
            <span className="font-offbit whitespace-nowrap">NORA AI</span>
          </div>
        </div>

        {/* Mode Switcher & Voice Selector */}
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 max-w-full overflow-x-auto no-scrollbar">
          <div className="flex items-center border border-white/15 p-0.5 bg-white/5 text-xs rounded-full backdrop-blur-md shrink-0">
            <button
              type="button"
              onClick={() => setIsLiveVoiceMode(false)}
              className={`px-2 sm:px-3.5 py-1 sm:py-1.5 transition-all flex items-center gap-1 sm:gap-1.5 rounded-full text-xs font-medium shrink-0 ${
                !isLiveVoiceMode
