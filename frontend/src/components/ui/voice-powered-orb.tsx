...
interface VoicePoweredOrbProps {
  ...
  /** External audio level to drive the orb (0-1). Overrides mic when provided. */
...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  const externalLevelRef = useRef<number | undefined>(externalAudioLevel);
  const isMicInitRef = useRef<boolean>(false);

  useEffect(() => {
    externalLevelRef.current = externalAudioLevel;
  }, [externalAudioLevel]);
  ...
  const stopMicrophone = () => {
    ...
    try {
      ...
      console.warn('stopMicrophone error:', e);
    }
  };
  ...
  // Microphone lifecycle effect — single authoritative source for mic start/stop
  useEffect(() => {
    if (enableVoiceControl && externalAudioLevel === undefined) {
      initMicrophone();
    } else {
      stopMicrophone();
    }
  ...
  }, [enableVoiceControl, externalAudioLevel]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const update = (t: number) => {
        ...
        const extLevel = externalLevelRef.current;
        if (extLevel !== undefined) {
          voiceLevel = extLevel;
          if (onVoiceDetected) onVoiceDetected(voiceLevel > 0.1);
        } else if (enableVoiceControl && isMicInitRef.current) {
        ...
        else currentRot += dt * baseRotationSpeed * 0.5;
